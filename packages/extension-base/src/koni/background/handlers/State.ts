...
import { isSubscriptionRunning, unsubscribe } from 'background/handlers/subscriptions';
import { AccountRefMap, AddTokenRequestExternal, AmountData, APIItemState, ApiMap, AuthRequestV2, BalanceItem, BalanceJson, BasicTxErrorType, ChainStakingMetadata, ChainType, ConfirmationsQueue, CrowdloanItem, CrowdloanJson, CurrentAccountInfo, EvmProviderErrorType, EvmSendTransactionParams, EvmSendTransactionRequest, EvmSignatureRequest, ExternalRequestPromise, ExternalRequestPromiseStatus, ExtrinsicType, MantaAuthorizationContext, MantaPayConfig, MantaPaySyncState, NftCollection, NftItem, NftJson, NominatorMetadata, RequestAccountExportPrivateKey, RequestCheckPublicAndSecretKey, RequestConfirmationComplete, RequestSettingsType, ResponseAccountExportPrivateKey, ResponseCheckPublicAndSecretKey, ServiceInfo, SingleModeJson, StakingItem, StakingJson, StakingRewardItem, StakingRewardJson, StakingType, UiSettings } from 'background/KoniTypes';
import { AccountJson, RequestAuthorizeTab, RequestRpcSend, RequestRpcSubscribe, RequestRpcUnsubscribe, RequestSign, ResponseRpcListProviders, ResponseSigning } from 'background/types';
import { ALL_ACCOUNT_KEY, ALL_GENESIS_HASH, MANTA_PAY_BALANCE_INTERVAL } from 'constants';
import { BalanceService } from 'services/balance-service';
import { ServiceStatus } from 'services/base/types';
import { ChainService } from 'services/chain-service';
import { _DEFAULT_MANTA_ZK_CHAIN, _MANTA_ZK_CHAIN_GROUP, _PREDEFINED_SINGLE_MODES } from 'services/chain-service/constants';
import { _ChainState, _NetworkUpsertParams, _ValidateCustomAssetRequest } from 'services/chain-service/types';
import { _getEvmChainId, _getSubstrateGenesisHash, _getTokenOnChainAssetId, _isAssetFungibleToken, _isChainEnabled, _isChainTestNet, _isSubstrateParaChain, _parseMetadataForSmartContractAsset } from 'services/chain-service/utils';
...
import { AuthUrls, MetaRequest, SignRequest } from 'services/request-service/types';
import SettingService from 'services/setting-service/SettingService';
import DatabaseService from 'services/storage-service/DatabaseService';
import { SubscanService } from 'services/subscan-service';
import { SUBSCAN_CHAIN_MAP_REVERSE } from 'services/subscan-service/subscan-chain-map';
import TransactionService from 'services/transaction-service';
import { TransactionEventResponse } from 'services/transaction-service/types';
import WalletConnectService from 'services/wallet-connect-service';
import AccountRefStore from 'stores/AccountRef';
import { stripUrl } from 'utils';
import { isContractAddress, parseContractInput } from 'utils/eth/parseTransaction';
import { createPromiseHandler } from 'utils/promise';
import { MetadataDef, ProviderMeta } from '@soul-wallet/extension-inject/src/types';
