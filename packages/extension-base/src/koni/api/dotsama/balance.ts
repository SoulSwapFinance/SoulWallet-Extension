...
import { APIItemState, BalanceItem, TokenBalanceRaw } from 'background/KoniTypes';
import { ASTAR_REFRESH_BALANCE_INTERVAL, SUB_TOKEN_REFRESH_BALANCE_INTERVAL } from 'constants';
import { PalletNominationPoolsPoolMember } from 'koni/api/staking/bonding/utils';
import { getEVMBalance } from 'koni/api/tokens/evm/balance';
import { getERC20Contract } from 'koni/api/tokens/evm/web3';
import { getPSP22ContractPromise } from 'koni/api/tokens/wasm';
import { getDefaultWeightV2 } from 'koni/api/tokens/wasm/utils';
import { state } from 'koni/background/handlers';
import { _BALANCE_CHAIN_GROUP, _MANTA_ZK_CHAIN_GROUP, _PURE_EVM_CHAINS, _ZK_ASSET_PREFIX } from 'services/chain-service/constants';
import { _EvmApi, _SubstrateApi } from 'services/chain-service/types';
import { _checkSmartContractSupportByChain, _getChainNativeTokenSlug, _getContractAddressOfToken, _getTokenOnChainAssetId, _getTokenOnChainInfo, _isChainEvmCompatible, _isPureEvmChain, _isSubstrateRelayChain } from 'services/chain-service/utils';
import { categoryAddresses, sumBN } from 'utils';
